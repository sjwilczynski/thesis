...
\begin{figure}
\centering
\includegraphics[width=0.9\textwidth]{images/SAMscores.png}
\caption{Scores of LR using features selected by SAM}
\label{fig:SAMscores}
\end{figure}

In the figure \ref{fig:SAMscores} we can see ROC AUC scores on test set for logistic regression model fitted using different number of variables selected by SAM. These numbers appear on the x axis. The red dot corresponds to $\Delta = 2.4571$ - lowest value for which $90$-th percentile of estimated FDR was $0$, indicating that all selected genes are influencing the response. However, for $\Delta = 2.4571$ $1266$ features were selected. It suggests that the result is not plausible, as that many genes are not probable to impact the disease. Nonetheless, there few interesting things to notice. First of all, ROC AUC score increases with the number of variables and the classifiers performed worse than PLS with LR or pure SVC and as expected feature selection yielded inferior performance to feature extraction. What is more, in order to achieve decent performance of ROC AUC equal to $0.782$ one has to use only $82$ genes. Even more interestingly to obtain performance of $0.747$ one need only $13$ genes, namely LRIG1, ABAT, EVL, ELOVL5, IL6ST, TBC1D9, KIF13B, SIK3, MYB, CST3, FOS, CIRBP, SCUBE2. They might be really good starting point for professionals trying to discover the genes responsible for the disease development. Also, when compared with mean performance of LR classifier using random subset of 710 genes from section \ref{section:exploratory}, we can see that only $82$ genes are needed to achieve it, indicating that genes selected by SAM are influential.
