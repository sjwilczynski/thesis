...
The other supervised dimensionality reduction method, that performs classification as well, namely Forest Deep Neural Network (\cite{fDNN}) is much more novel. This method aims at exploiting neural networks, one of the most powerful, state-of-the-art family of models applicable in the field of classification, in analysis of high dimensional data. \textcolor{blue}{opis neuronow} Neural networks usually do not work in the $n \ll p$ setting as in order to capture the complicated correlations within the data they need to be trained using large number of samples that have to be significantly larger than the number of features. In order to tackle this issue a new representation of variables is created using Random Forest technique. The method fits Random Forest classifier consisting of $p^{\prime}$ decision trees $T_1, \ldots, T_{p^{\prime}}$. Let $T_i(x_j)$ be the label assigned to $j$-th sample by $i$-th tree. Then the new representation of $j$-th sample is $x_j^{\prime} = \left(T_1(x_j), \ldots, T_{p^{\prime}}(x_j)\right)$ and the reduced data matrix is $X^{\prime}$. For purpose of our analysis we will use neural network consisting of two hidden layers. The flow of data through the network can be expressed as:
\begin{align*} 
