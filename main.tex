...
\begin{align*}
    ...
    Lap(0, b) &= \frac{1}{2 b} \exp \left(-\frac{|x|}{b}\right)
\end{align*}
...
as increase in either of $\lambda$ magnifies the effect of regularization. This two schemes are called $L_2$ and $L_1$ regularizations respectively and the latter term in both equations is called penalty.
...
\end{align*}
The value of $\lambda$ is chosen using Ledoit-Wolf lemma (\cite{honey}) and $\alpha$ is the average eigenvalue of sample covariance matrix. In summary, RDA is a simple classifier, that uses Mahalanobis metric to measure distances from mean classes and regularized sample covariance matrix. The linear decision boundary is also appealing as it leads to model that is easy to interpret.
